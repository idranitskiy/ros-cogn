    ...
    filename_(filename)
  {
    ...
    if (repeat_delay_ > 0.0)
      ROS_INFO("Delay %.3f seconds before repeating input file.",
               repeat_delay_);
    if (play_season_)
      ROS_INFO("Playing season from %.3d to %.3d episode", start_, end_);
    ...
    if(play_season_)
    {
      ...
      if ((start_ > end_)||(start_<1))
      {
        ROS_FATAL("Bad episode numbers");
        return;
      }
      episode = start_;
      ...
      ss << std::setw(3) << std::setfill('0') << episode;
      ss >> str_episode;

      season += str_episode + ".pcap";

      ROS_INFO("Opening PCAP file \"%s\"", season.c_str());   

      if ((pcap_ = pcap_open_offline(season.c_str(), errbuf_) ) == NULL)
      {
        ROS_FATAL("Error opening Velodyne socket dump file((( \"%s\"", season.c_str());
        return;
      }
    }
    else
	...
	if (play_season_)
        {
          ...
          if (episode >= end_)
          {
            ...
            episode++;
          ROS_INFO("next episode %d", episode);
          std::string season;
          std::string str_episode;
          std::stringstream ss;
          ...
          season += str_episode + ".pcap";
          ROS_INFO("next link %s", season.c_str());
          ROS_INFO("Opening PCAP file \"%s\"", season.c_str());   
